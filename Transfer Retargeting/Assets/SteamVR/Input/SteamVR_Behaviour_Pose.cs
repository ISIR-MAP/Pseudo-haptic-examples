...
namespace Valve.VR
{
    ...
    public class SteamVR_Behaviour_Pose : MonoBehaviour
    {
        ...
        Vector3 warpOrigin, warpTargetReal, warpTargetVirtual;
        bool warping = false;
        ...
        protected virtual void Update()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                warping = !warping;
                if (warping) {
                    //fixedPoint.transform.localScale += new Vector3(warpRatio, 0f, warpRatio);
                } else {
                    //fixedPoint.transform.localScale -= new Vector3(warpRatio, 0f, warpRatio);
                }
            }
            ...
            {
                world.transform.position = origin.transform.TransformPoint(poseAction[inputSource].localPosition);
            }
        }
        ...
        protected virtual void UpdateTransform()
        {
            ...
            if (origin != null)
            {
                Vector3 virtHandPos = origin.transform.TransformPoint(poseAction[inputSource].localPosition);

                if (warping)
                {
                    virtHandPos.x += (virtHandPos.x - fixedPoint.transform.position.x) * warpRatio;
                    virtHandPos.z += (virtHandPos.z - fixedPoint.transform.position.z) * warpRatio;
                }
                print(virtHandPos.x + " " + virtHandPos.y + " " + virtHandPos.z);

                transform.position = virtHandPos;
